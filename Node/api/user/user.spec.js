...
describe('GET /users는', () => {
  describe('성공시', () => {
    const users = [{ name: 'alice' }, { name: 'bak' }, { name: 'chris' }];
    before(() => {
...
describe('delete /users/1', () => {
  ...
  describe('실패 시', () => {
    it('id가 숫자가 아닐경우 404로 응답', (done) => {
      request(app).delete('/users/two').expect(400).end(done);
    });
  });
});
...
describe('POST /users', () => {
  ...
  describe('실패 시', () => {
    ...
    it('name이 중복됬을 경우 409을 반환한다.', (done) => {
...
describe.only('PUT /user/:id', () => {
  ...
  describe('실패 시', () => {
    ...
    it('이름 이 중복일 경우 409를 응답한다.', (done) => {
      request(app).put('/users/3').send({ name: 'bak' }).expect(409).end(done);
    });
  });
});
